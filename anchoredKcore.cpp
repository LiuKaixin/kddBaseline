...
vector<long> subgraphTag, subgraphDegree, verDegree, anchorTag, followTag;//verDegree保存每个节点的度
...
//kmbTag为每个点初始化为1，当节点为删除则为0，当节点是(k-b)-core的邻居的话为-1, .kTag为每个点初始化为1.
vector<long> followers, anchoredTag, followersRecord;
vector<long> verSetIndex, setTagIDs;//verSetIndex 取verSet的第一个元素的序列值，setTagIDs度小于K-B的节点标签。
...
void dataInput()//读入数据保存到verSet（二维向量）中，对二维向量的第一列做索引存在verSetIndex中，初始化一系列变量，并且对图的度进行统计。
{
	...
	{
		while (fscanf(fe, "%ld%ld", &vertexID, &neighborID) != EOF) {
			if (verSet.size() == 0)
			{
			...
			{
				if (vertexID == verSet[verSet.size() - 1][0])//这里要求边的文件按照源节点排序。
				{
...
void computeKmbKcore()//计算(k-b)-core和k-core
{
	for (unsigned long i = 0; i < setTagIDs.size(); i++)//对度小于K-B的节点直接排除，并更新相关变量即kmb系列
	{
	...
	for (unsigned long i = 0; i < kDegree.size(); i++)//再次检测并将度小于K的点的tag设置为2
	{
...
void anchorOneV6SRecord(long anc, long maxSize)
{
    ...
    //printf("a:%ld\t%ld\t\t%ld:\t",anc,verSet[anc][0],anchorNumber);
...
void algorithm()
{
	...
	kmbcoreNumber = kmbTag.size();//这里还是节点数量呢
	//get (k-b)-core and k-core
	computeKmbKcore();
	ofstream kTagfile;
	kTagfile.open("ktag.txt",ios::app);
	/*输出kTag
	for(int temp=0;temp<kTag.size();temp++){
	    long verSetID=verSet[temp][0];
	    int type=kTag[temp];
        //kTagfile<<verSetID<<"\t"<<type<<"\n";
        printf("%d\t%d\t%d\n",verSetID,temp,type);
	}
	 */
	kTagfile.close();
	...
	for (unsigned long i = 0; i < kmbTag.size(); i++)
	{
		if (kmbTag[i] == 1)
		{
			for (unsigned long j = 1; j < verSet[i].size(); j++)
			{
				long neiVertexID = verSet[i][j];
				long neiverSetID = verSetIndex[neiVertexID];
				if (verSet[neiverSetID][0] == neiVertexID && !kmbTag[neiverSetID])//如果邻居已被删除
				{
	...
	for (unsigned long i = 0; i < kmbNeighbors.size(); i++)
	{
		...
		for (unsigned long j = 1; j < verSet[id].size(); j++)
		{
			long neiVertexID = verSet[id][j];
			long neiverSetID = verSetIndex[neiVertexID];
			if (verSet[neiverSetID][0] == neiVertexID && kmbTag[neiverSetID])//如果(k-b)-core的邻居是
			{
	...
	for (long i = 0; i < inputB; i++)
	{
		...
		if (i%100==99){
            cout<<"B:"<<i<<"\tK:"<<anchorAndfollowerNumber - i-1<<endl;
            runTime = (double)clock() / CLOCKS_PER_SEC - algStartTime;
            printf("time: %lf\n", runTime);
            followers_times.push_back(make_pair(anchorAndfollowerNumber - i-1,runTime));
		}
	}
...
void dataOutput()
{
	...
	{
		sprintf(record, "file%s", infile.c_str());
...
int main(int argc, char *argv[])
{
	...
	infile = "dataset.txt";
	infile = "../dataset/facebook_combined.txt";
	//infile = "../dataset/lj/graph.txt";
	outfile = "result.txt";
	//scanf("%ld %ld", &inputK, &inputB);

	int o;
	const char *optstring = "f:k:b:"; // 有三个选项-abc，其中c选项后有两个冒号，表示后面可选参数
